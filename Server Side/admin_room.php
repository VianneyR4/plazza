<?php
	...
	if (isset($_POST['submit_room'])){
		...
		if (!empty($_FILES['files']['name'][0])) {
			...
			$query_upload_room = $db->prepare('INSERT INTO chambres(disponibilite,type,numero,standard,stars,description,wifi,tv,prix) VALUES(:disponibilite,:type,:numero,:standard,:stars,:description,:wifi,:tv,:prix)');
			...
			foreach($files['name'] as $position => $file_name){
				 ...
				 if (in_array($file_ext, $allowed)){
					 ...
					 $failed[$position] = "[{$file_name}] les fichiers '{$file_ext}' ne sont pas autorisé!";	
				 }
			}
